...
class TestWebApp(unittest.TestCase):
    ...
    def populate_db(self):
        ...
        for setting in default_settings:
            db.session.add(
                Settings(key=setting, value=default_settings[setting], user_id=1)
            )
        for setting in default_settings:
            db.session.add(
                Settings(key=setting, value=default_settings[setting], user_id=2)
            )
        db.session.add(user)
    ...
    def test_register_user(self):
        ...
        assert response.status_code == 200
        assert response.request.path == '/explore'
